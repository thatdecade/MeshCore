...
void DataStore::loadPrefsInt(const char *filename, NodePrefs& _prefs, double& node_lat, double& node_lon) {
  ...
  if (file) {
    ...
    if (file.available() >= (int)sizeof(_prefs.auto_advert_reply_on_rx)) {
      file.read((uint8_t *)&_prefs.auto_advert_reply_on_rx, sizeof(_prefs.auto_advert_reply_on_rx)); // 90
    }
...
void DataStore::savePrefs(const NodePrefs& _prefs, double node_lat, double node_lon) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs.auto_advert_reply_on_rx, sizeof(_prefs.auto_advert_reply_on_rx)); // 90
